...
export default function CaseDetailPage() {
  ...
  const [updateHistory, setUpdateHistory] = useState<Array<{ author: string; role: string; timestamp: string; message: string }>>([]);
  ...
  const fetchCase = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setEditedDepartment(data.incident.assignedDepartment || "");
        setUpdateHistory([
          {
            author: "Sistema",
            role: "Automático",
            timestamp: new Date(data.incident.createdAt).toLocaleString("es-VE"),
      ...
      if (data.success) {
        ...
        setUpdateHistory((prev) => [
          ...prev,
          {
            ...
            message: `Estado cambiado a ${STATUS_LABELS[data.incident.status as ValidStatus] || data.incident.status}.`,
          },
        ]);
        setIsEditing(false);
      }
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 flex justify-end">
                    <button onClick={handleAddUpdate} disabled={!newUpdate.trim()} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
