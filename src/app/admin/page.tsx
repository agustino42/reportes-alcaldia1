...
export default async function AdminDashboard() {
  ...
  const pendientes = allIncidents.filter((i) => i.status === "RECIBIDO").length;
  const enProceso = allIncidents.filter((i) => i.status === "EN_REVISION").length;
  const resueltos = allIncidents.filter((i) => i.status === "COMPLETADO").length;
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-200 flex justify-between items-center bg-slate-50/50">
          <h3 className="text-lg font-bold text-slate-800">
            Requerimientos Recientes ({incidents.length})
          </h3>
        </div>
                ...
                incidents.map((incident) => (
                  <tr key={incident.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <p className="text-sm text-slate-500 mt-1 truncate max-w-[250px] flex items-center gap-2">
                    ...
                    <td className="px-6 py-4 text-slate-600 text-sm whitespace-nowrap font-medium hidden md:table-cell">
                      <div className="flex items-center gap-1">
                        <Calendar className="w-3.5 h-3.5" />
                        {incident.createdAt.toLocaleDateString("es-VE", {
                          day: "numeric",
                          month: "short",
                          year: "numeric",
                        })}
                    ...
                    <td className="px-6 py-4 hidden sm:table-cell">
                      <p className="text-sm font-semibold text-slate-800 flex items-center gap-1">
                        ...
                        {incident.citizenName || "Anónimo"}
                      </p>
                      {incident.citizenDni && <p className="text-xs text-slate-500 mt-0.5">{incident.citizenDni}</p>}
                    </td>
                    <td className="px-6 py-4">
